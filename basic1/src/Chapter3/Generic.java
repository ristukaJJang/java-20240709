...
// Test4 형태의 클래스로 작성하면 코드의 중복은 해결할 수 있지만 사용시 타입의 안정성이 떨어짐
class Test4 {
	String subject;
	Object score;
}
...
// T extneds 클래스명 : 특정 클래스를 상속한 클래스만 제너릭 타입으로 받을 수 있도록 제한
...
public class Generic {
	...
	// ? : 제너릭으 ㅣ와이들 카드로 모든 타입을 받을 수 있도록 함
	// 와일드 카드가 사용되는 제너릭은 메서드의 매개변수 타입이나, 반환타입에 사용
	...
	// ? super 클래스명 : 클래스 혹은 특정 클래스의 상위 클래스만 타입으로 받을 수 있도록 제한
	...
	public static void main(String[] args) {
		...
		// MyClass or MyClass를 상속한 클래스만 매개변수로 전달 가능
		extendsMethod(list4);
		extendsMethod(list5);
		//extendsMethod(list3);
		//extendsMethod(list2);
		...
		// MyClass or MyClass의 부모 클래스만 배개변수로 전달할수 있다
		superMethod(list4);
		superMethod(list3);
		// superMethod(list5);
		// superMethod(list2);
