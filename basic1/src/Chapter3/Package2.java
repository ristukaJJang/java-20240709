...
public class Package2 {
	...
	public static void main(String[] args) {
		...
		list.add(99);
		list.add(100);
		list.add(null); //list[2] 출력하면 에러뜸
		
		System.out.println(list);
		
		// get() : list의 특정 인덱스 값을 가져옴
		// list의 인덱스 범위를 초과하지 않도록 주의
		// 기본형 데이터 타입 변수로 반환값을 받을 땐 널이 올 수 있음을 주의(NullPointerException)
		...
		stringList.remove("melon");
		System.out.println(stringList);
		
		//size() : list 크기 반환
		System.out.println(stringList.size());
		
		// Set : 중복을 허용하지 않고 순서가 존재하지 않는 데이터 구조(집합)
		Set<String> set;
