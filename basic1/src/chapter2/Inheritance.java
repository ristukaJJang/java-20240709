...
// class SubClass exteneds SuperClass {...}
...
// 자바에서는 단일 상속만 지원
// 다중 상속에서 발생할 수 있는 다이아몬드 문제 발생 예방
// 다중 상속과 동일한 효과를 보기 위해서 인터페이스 혹은 중첩 상속 수행
class Developer extends Human /*, Korean1 -> 불가*/{
	String[] skills;
	
	Developer(String name, int age, String[] skills) {
		// super : 부모 클래스를 지칭하는 키워드
		// super()는 부모 클래스의 생성자를 지정
		super(name, age);
		this.skills = skills;
	}
...
class SalesMan extends Human{
	...
	SalesMan(String name, int age, int sales) {
		// 부모 생성자 호출은 반드시 현재 생성자의 첫번재 구문으로 작성해야 함
		super(name, age);
		this.age = age;
	}
	 ...
	 * 규칙
	 * 1. 상위 클래스에서 선언된 메서드와 메스드 이름, 매개 변수의 조합이 동일해야 한다
	 * 2. 상위 클래스에서 선언된 메서드가 final로 선언되면 재정의 불가능
	 * */
	// @override : 컴파일러에게 현재 작업이 오버라이딩 작업임을 알려줌
	@Override
	//final로 선언된 메소드는 오버라이딩 할 수 없음
	void sayHello() {
	...
	void sayHello(String company) {
		System.out.println(name + "이/가" + company + "에서 자신을 소개합니다.");
	}
...
//	}
	...
	void sale() {
		System.out.println(name + "가 " + sales + "만큼의 매출을 달성했습니다.");
	}
}
...
public class Inheritance {
	...
	public static void main(String[] args) {
	    ...
	    SalesMan salesMan1 = new SalesMan("이영희", 20, 300000);
	    ...
	    // 모든 클래스는 'object'라고 하는 클래스를 최상위 클래스로 반드시 상속받고 있음
